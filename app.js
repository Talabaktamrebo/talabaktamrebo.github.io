...
function renderFreeAdTerms(){
  ...
  if(termsAccepted('freead')){
    box.innerHTML='<div class="terms-note">بإرسال إعلانك فإنك توافق على <a class="terms-link" onclick="openFreeAdTermsModal()">شروط النشر وسياسة الخصوصية</a> لطلبك تم.</div>';
  }else{
    box.innerHTML='<p style="font-size:13px;color:#475569;margin:0 0 10px">قسم مجاني بالكامل لدعم أصحاب المهن والأسر المنتجة — بلا رسوم ولا عمولة.</p>'
      +'<div class="terms-agree-one"><label class="fa-term"><input type="checkbox" id="faAgree" class="fa-term-chk" onchange="faTermsCheck()"><span>أوافق على <a class="terms-link" onclick="openFreeAdTermsModal()">شروط النشر وسياسة الخصوصية</a> لطلبك تم</span></label></div>'
...
function openFreeAdModal() {
  ...
  faTermsCheck();
  backToTerms();
  if(termsAccepted('freead')) showFreeAdForm();   // تخطّي الشروط بعد الموافقة السابقة
}
// تفعيل زرّ «متابعة» بعد الموافقة (أو إن سبق ووافق)
function faTermsCheck() {
  var agree = document.getElementById('faAgree');
  var ok = termsAccepted('freead') || (agree && agree.checked);
...
function showFreeAdForm() {
  setTermsAccepted('freead');   // تذكّر الموافقة بعد المتابعة لأول مرة
...
/* ===== الموافقة على شروط النشر: مربّع واحد + تذكّر الموافقة على الجهاز ===== */
var TERMS_VER='1';   // عند تغيير الشروط مستقبلاً ارفع الرقم ليُعاد طلب الموافقة مرّة واحدة
function termsAccepted(k){ try{ return localStorage.getItem('tam_terms_'+k)===TERMS_VER; }catch(e){ return false; } }
function setTermsAccepted(k){ try{ localStorage.setItem('tam_terms_'+k, TERMS_VER); }catch(e){} }
window.termsAccepted=termsAccepted; window.setTermsAccepted=setTermsAccepted;
...
function openTermsModal(html, title){
  ...
  if(!m){
    ...
    m.innerHTML='<div class="pub-terms-card"><div class="pub-terms-head"><h3 id="pubTermsTitle">شروط النشر وسياسة الخصوصية</h3><button class="pub-terms-x" onclick="closeTermsModal()" aria-label="إغلاق">✕</button></div><div class="pub-terms-body" id="pubTermsBody"></div><div class="pub-terms-foot"><button class="sell-btn primary" onclick="closeTermsModal()">حسناً، فهمت</button></div></div>';
...
function renderSellTerms(){
  ...
  if(termsAccepted('sell')){
    // وافق مسبقاً: لا مربّعات — سطر صغير + رابط لعرض الشروط
    box.innerHTML='<div class="terms-note">بإرسال إعلانك فإنك توافق على <a class="terms-link" onclick="openSellTermsModal()">شروط النشر وسياسة الخصوصية</a> لطلبك تم.</div>';
  }else{
    // أول مرة: مربّع موافقة واحد + رابط لقراءة الشروط كاملة (اختياري)
    box.innerHTML='<div class="terms-agree-one"><label class="fa-term"><input type="checkbox" id="sellAgree" class="sell-term-chk" onchange="sellTermsCheck()"><span>أوافق على <a class="terms-link" onclick="openSellTermsModal()">شروط النشر وسياسة الخصوصية</a> لطلبك تم</span></label></div>';
  }
  sellTermsCheck();
}
function sellTermsCheck(){
  ...
  if(ok && agree && agree.checked) setTermsAccepted('sell');   // تذكّر الموافقة لأول مرة
  // عند الموافقة: يختفي مربّع الموافقة (لا السطر التذكيري) ويظهر الفورم
...
function renderPartnerTerms(){
  ...
  if(termsAccepted('partner')){
    box.innerHTML='<div class="terms-note">بإرسال طلبك فإنك توافق على <a class="terms-link" onclick="openPartnerTermsModal()">شروط النشر وسياسة الخصوصية</a> لطلبك تم.</div>';
  }else{
    box.innerHTML='<p style="font-size:13px;color:#475569;margin:0 0 10px">قسم مجاني لربط أصحاب الأفكار والأصول ورؤوس الأموال بالشركاء.</p>'
      +'<div class="terms-agree-one"><label class="fa-term"><input type="checkbox" id="partnerAgree" class="partner-term-chk" onchange="partnerTermsCheck()"><span>أوافق على <a class="terms-link" onclick="openPartnerTermsModal()">شروط النشر وسياسة الخصوصية</a> لطلبك تم</span></label></div>';
  }
}
...
function partnerTermsCheck(){
  ...
  var ok = termsAccepted('partner') || (agree && agree.checked);
  if(ok && agree && agree.checked) setTermsAccepted('partner');   // تذكّر الموافقة لأول مرة
  // عند الموافقة (أول مرة): يختفي مربّع الموافقة ويظهر الفورم؛ إن وافق مسبقاً يبقى السطر التذكيري ظاهراً
